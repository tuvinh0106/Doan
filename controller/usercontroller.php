...
function Change()
{
    if (isset($_SESSION['user_name'])) {
        ...
        session_destroy();
        header("Location: ../view/index.php?Message=Change password successfull !!! Please sign in !!!");
    }
}
...
function signUp($user)
{
    ...
    $_SESSION["email"] = $email;
    header("Location: ../index.php?Message=Wellcome to our testsite !!!");
}
...
function changePass($new_pass, $email)
{
    ...
    $stmt->execute();
    header("Location: ../view/thongtincanhan.php?Message=Change password successfull !!!");
}
function update($diachi, $sdt, $email)
{
    $stmt = connectDatabase()->prepare("UPDATE user set DiaChi='$diachi',Sdt='$sdt' WHERE Email = '$email'");
    $stmt->execute();
    header("Location: ../view/thongtincanhan.php?Message=Update Information successfull !!!");
}
    ...
    case $_POST['user'] == 'changepass':
        ...
        if ($confirm == $new_pass) {
            if (checkPass($old_pass, $email)) {
                ...
                header("Location: ../view/thongtincanhan.php?Message=Incorrect password !!!");
            }
        }
        break;
